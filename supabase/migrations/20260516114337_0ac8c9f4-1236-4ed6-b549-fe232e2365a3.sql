  ...
  LOOP
    ...
    ELSIF r.type = 'true_false' THEN
      DECLARE
        ...
        v_score numeric := 0;
      BEGIN
        -- Prefer explicit is_correct flags on options
        SELECT ARRAY(SELECT id FROM public.mcq_options WHERE question_id = r.question_id AND is_correct)
          INTO v_correct;
        -- Fallback: derive from meta.correct (boolean) by matching option body 'true'/'false'
        IF v_correct IS NULL OR array_length(v_correct,1) IS NULL THEN
          ...
          IF v_correct_bool IS NOT NULL THEN
            ...
            SELECT ARRAY(SELECT id FROM public.mcq_options
                         WHERE question_id = r.question_id AND lower(btrim(body)) = v_correct_label)
              INTO v_correct;
          END IF;
        END IF;
        IF v_correct IS NOT NULL AND array_length(v_correct,1) = 1
           AND array_length(v_selected,1) = 1
    ...
    ELSIF r.type = 'short_answer' THEN
      DECLARE
        ...
        v_score numeric := 0;
      BEGIN
        -- Normalize: trim, collapse internal whitespace, optionally lowercase
        ...
        FOR v_acc IN SELECT jsonb_array_elements_text(COALESCE(r.meta->'accepted', '[]'::jsonb)) LOOP
          ...
          IF v_acc_norm = v_norm AND v_norm <> '' THEN
            v_match := true; EXIT;
          END IF;
        END LOOP;
